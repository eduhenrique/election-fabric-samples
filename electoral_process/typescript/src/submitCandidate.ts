...
async function main() {
    try {
        ...
        //submitCandidate(ctx: Context, participanteNumber: string, cargoNumber: string)
        //COMO EU SEI QUE O USER UTILIZADO É O PARTICIPANTE 0, 1, 2? COmo associar user com um asset?
        const result0 = await contract.submitTransaction('submitCandidate', 'POSITION0', 'XD - Prometo Animes de qualdiade para todos.');
        console.log(`Candidate on position0 has been submitted - `+`${result0.toString()}\n`);
        // const result1 = await contract.submitTransaction('submitCandidate', 'PARTICIPANTE1', 'CARGO1', '');
        // console.log(`Candidato no cargo1 has been submitted - `+`${result1.toString()}\n`);
        // const result2 = await contract.submitTransaction('submitCandidate', 'PARTICIPANTE2', 'CARGO1', 'Será feito um programa de inclusão social para a organização.');
